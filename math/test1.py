...
def _request_llm_for_trans(API_KEY, Base_url, system_prompt, user_prompt):
    ...
    # 发送请求
    response = requests.post(Base_url, json=payload, headers=headers, timeout=200)
...
# 循环处理每个问题
for problem in amc_problems[:100]: 
    ...
    while True:
        try:
            ...
            if "<answer>" in response_content:
                final_answer =response_content.replace("<answer>", "").strip()
                break
