...
system_prompt = """Let's think step by step and output the final answer.Please provide the final answer enclosed exactly within <answer> tags. Do not include any additional text after the answer. """
...
def request_llm(system_prompt, user_prompt):
    ...
    response = requests.post(base_url, json=payload, headers=headers, timeout=2000)
...
def process_difficulty_level(difficulty):
    """处理单个难度级别的问题集"""
    filename = f"difficulty_{difficulty}.json"
...
def main():
    ...
    # 保存结果到文件
    results_file = "difficulty_metrics.json"
